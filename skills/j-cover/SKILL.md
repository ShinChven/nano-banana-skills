---
name: j-cover
description: Analyzes and reconstructs J-Media and Gravure cover designs using a 4-axis structural layering approach.
version: 1.0.0
---

# ROLE: Expert Magazine Layout Designer & Image Editor

# INPUT IMAGE: [Provided Image]

# STEP 1: PRECISE OUTPAINTING & COMPOSITION
* **Action:** ZOOM OUT SLIGHTLY (Scale down the original subject by exactly 20%).
* **Canvas:** Maintain the ORIGINAL Aspect Ratio of the input image.
* **Anchor:** Position the original character at the BOTTOM-CENTER of the frame.
* **Fill:** Seamlessly generate (outpaint) the background in the newly created 20% margin around the top and sides. Match the original depth of field and lighting perfectly.
* **Constraint:** The character's face and details must remain IDENTICAL, just slightly smaller to create breathing room for text.

# STEP 2: TYPOGRAPHY & LAYOUT
* **Auto-Palette:** Sample the dominant color from the character's outfit (e.g., Deep Red or Gold) for the Main Title font.

* **Main Title (Japanese):**
    * Content: Generate a 4-character Kanji title (e.g., "古都の華").
    * Placement: Place it in the newly created empty space at the VERY TOP. It can slightly overlap the hair but **MUST NOT** cover the face.

* **Subtitles (Japanese):**
    * Content: Vertical text describing the mood.
    * Placement: Floating in the new background space on the Left or Right sides.

* **Captions (English):**
    * Content: Short, engaging teasers (e.g., "Exclusive Interview," "Summer Look").
    * Placement: Horizontal text scattered in the remaining negative space (corners or near the shoulder).

# STEP 3: GRAPHIC ELEMENTS
* **Lore Icon:** Add a specific faction/game icon (e.g., YoRHa, Umbrella, Meteor) in a top corner.
* **Magazine Data:**
    * Barcode in bottom right.
    * Price: "¥980"
    * Date: "AUGUST 2025" (English)

# OUTPUT GOAL
A high-quality magazine cover scan. The character is still large and prominent (80% scale), with just enough new background space added to fit the Japanese Title on top and English Captions on the sides.
